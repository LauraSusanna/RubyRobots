class Field:
    ...
    def __init__(self, x, y):
        if self._coordinates_are_positive_ints_or_zero(x, y):
            self.max_x = x
            self.max_y = y
            self.currently_occupied_positions = []
        else:
            raise ValueError('Field coordinates invalid')
    ...
    def update_currently_occupied_positions(self, old_x, old_y, new_x, new_y):
        if [old_x, old_y] in self.currently_occupied_positions and self._is_valid_position(new_x, new_y):
    ...
    def _is_valid_position(self, x, y):
        if self._coordinates_are_positive_ints_or_zero(x, y):
            return self.is_on_field(x, y) and self.causes_no_collision(x, y)
        else:
            return False

    def _coordinates_are_positive_ints_or_zero(self, x, y):
        return isinstance(x, int) and isinstance(y, int) and x >= 0 and y >= 0

    def is_on_field(self, x, y):
